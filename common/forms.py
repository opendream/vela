...
class PermalinkForm(CommonForm):
    ...
    def clean(self):
        ...
        for field_name in self.PERMALINK_FIELDS:
            ...
            if self.model.objects.filter(**{'%s__iexact' % field_name: permalink}).exclude(id=self.inst.id).count() > 0:
...
class CommonModelForm(forms.ModelForm):
    ...
    def as_bootstrap(self):
        ...
        return self._html_output(
            normal_row='<div%(html_class_attr)s>%(label)s<div class="field-container">%(field)s%(help_text)s%(errors)s</div></div>',
            error_row='<li>%s</li>',
            row_ender='</div>',
